...
public class MyFoaDB{
	...
	double bestSmell = Function.ackley(bestFly) ;  //全局最优浓度保存
	int gen = 0 ;                                  //迭代次数
	...
	while(gen < maxGen){
		...
		double smell = Function.ackley(f) ;
		if(smell < bestSmell){
		    getBetter = true ;
		    bestSmell = smell ;
		    bestFly = f.clone() ;
		...
		for(Fly f : flyList){                     //更新更新群体位置
		    ...
		    for(int i = 0; i < coor.length; i ++){
			coor[i] = (bestCoor[i] + Math.random() * (b - a) + a ) * t;
		    }
		}
	    }
	    bestS[gen] = bestSmell ;
	...
	// double tmpSmell = Function.ackley(bestFly) ;
	// for(int i = 0; i < d.length; i ++){
	//     w[i] = Math.random() * 2 - 1 ;
	//     while(Math.abs(w[i]) > 10e-5){
	// 	d[i] += w[i] ;
	// 	if(Function.ackley(bestFly) < tmpSmell){
	// 	    d[i] -= w[i] ;
	// 	    break ;
	// 	}
	// 	d[i] -= w[i] ;
	...
	for(Fly f : flyList){           //对每一个果蝇的每一维进行试探训练
		...
		// coor[i] += weight[i] ;                       //.....
		// System.out.println(smell + ":" + Function.ackley(f) + ":" + 
		// 		   weight[i] + ":" + coor[i] + "i" + i) ;
		// coor[i] -= weight[i] ;
		//	scan.nextLine() ;
		// coor[i] += tmp ;
